...
function mapUserRow(row: TrackedUserRow, addresses: AddressInfo[]): User {
  return {
    ...
    addresses,
    totalAssetUsd: typeof row.total_asset_usd === 'number' ? row.total_asset_usd : 0,
    historicalMaxAssetUsd:
      typeof row.historical_max_asset_usd === 'number' ? row.historical_max_asset_usd : 0,
    assetUpdatedAt: typeof row.asset_updated_at === 'number' ? row.asset_updated_at : null,
...
function sanitizeUser(user: User): User {
  return {
    ...
    totalAssetUsd: typeof user.totalAssetUsd === 'number' ? user.totalAssetUsd : 0,
    historicalMaxAssetUsd:
      typeof user.historicalMaxAssetUsd === 'number' ? user.historicalMaxAssetUsd : 0,
    assetUpdatedAt: typeof user.assetUpdatedAt === 'number' ? user.assetUpdatedAt : null,
...
function upsertUserRow(user: User, now: number) {
  ...
  db.prepare(
    ...
    user.totalAssetUsd,
    Math.max(user.historicalMaxAssetUsd, user.totalAssetUsd),
    user.assetUpdatedAt,
    now,
    now
  );
